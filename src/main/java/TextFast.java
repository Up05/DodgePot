...
public class TextFast {
    ...
    public static void init(){
        if(font == null){
            try {
                font = ImageIO.read(Objects.requireNonNull(Main.instance.getClass().getResource("Lucida Console.png")));
...
//    public TextFast update(){ return this; }
    ...
    public TextFast setPosition(Vector2 position){
        x = (int) position.getX();
        y = (int) position.getY();
        return this;
    }
    ...
    public void draw(){
        int _y = y;
        if(shouldCenterY) _y -= newlineCount * 8; // technically incorrect, but I don't plan on doing this with multiple lines.
        int index = 0;
        ...
        for(int i = 0; i < text.length(); i ++){
            ...
            int _x = x + index * cs;
            if(shouldCenter) _x -= text.length() / newlineCount / 2 * cs;
    ...
    public TextFast undraw(){
        int _y = y;
        if(shouldCenterY) _y -= 8 * newlineCount;
        GL11.glColor3f(0.05f, 0.05f, 0.05f);
        int _w = (cs) * text.length() / newlineCount;
        if(!shouldCenter)
            Shapes.rect(x,             _y, _w, 16);
        else
            Shapes.rect(x - _w / 2, _y, _w, 16);
